...
class Node {
public:
  Node(Node *parent, const wxString &key, const wxString &type,
  ...
  NodePtrArray &GetChildren() { return m_children; }
  Node *GetNthChild(unsigned int n) { return m_children.at(n).get(); }
...
public:
  ...
  Node *AddRootEntry(const wxString &key, const wxString &type,
                     ...
                     const wxString &parent_type = "dict") const;
...
Model::Model() { m_root = new Node(nullptr, "Root", "Dictionary", ""); };
Node *Model::AddRootEntry(const wxString &key, const wxString &type,
                          const wxString &value,
                          const wxString &parent_type) const {
...
void Model::GetValue(wxVariant &variant, const wxDataViewItem &item,
...
wxDataViewItem Model::GetRoot() const { return wxDataViewItem((void *)m_root); }

bool Model::SetValue(const wxVariant &variant, const wxDataViewItem &item,
                     unsigned int col) {
  wxASSERT(item.IsOk());
...
enum { ID_FILE = 1, ID_NEW = 2 };

wxBEGIN_EVENT_TABLE(Frame, wxFrame) EVT_MENU(ID_FILE, Frame::OnFileOpen)
    EVT_MENU(ID_NEW, Frame::OnFileNew) EVT_MENU(wxID_EXIT, Frame::OnExit)
...
Node *AddNodes(Node *treeNode, rapidxml::xml_node<char> *node, std::string k) {
  int index = 0;
  if (k == "array") {
    for (rapidxml::xml_node<> *n = node->first_node(); n;
         n = n->next_sibling()) {
      std::string key = std::string(n->name());
      std::string value = n->value();
      value = trim(value);

      Node *tn = treeNode->AddEntry(
          std::to_string(index), getDisplayType(n->name()), value, n->name());
    ...
    for (rapidxml::xml_node<> *n = node->first_node(); n;
      ...
      if (key != "key") {
        ...
        Node *tn = treeNode->AddEntry(n->value(),
                                      getDisplayType(n->next_sibling()->name()),
                                      value, n->next_sibling()->name());
...
Frame::Frame(const wxString &title, const wxPoint &pos, const wxSize &size)
  ...
  dataview->AppendTextColumn("Key", 0, wxDATAVIEW_CELL_EDITABLE,
                             (40.0 / 100.0) * size.GetWidth());
...
void Frame::OnAbout(wxCommandEvent &event) {
  wxMessageBox("Qlist is a FOSS property list (plist) editor written in C++", "About Qlist",
               wxOK | wxICON_INFORMATION);
}
void Frame::OnFileOpen(wxCommandEvent &event) {
  model->DeleteAll();
  wxFileDialog openFileDialog(this, _("Open Property-List file"), wxEmptyString,
                              wxEmptyString, _("Property-List file|*.plist"),
                              wxFD_OPEN | wxFD_FILE_MUST_EXIST);
  if (openFileDialog.ShowModal() == wxID_OK) {

    std::ifstream PlistFile(openFileDialog.GetPath());
    std::vector<char> buffer((std::istreambuf_iterator<char>(PlistFile)),
    ...
    for (rapidxml::xml_node<> *node = root->first_node(); node;
      ...
      if (std::string(root->name()) == "array") {
        Node *treeNode = model->AddRootEntry(std::to_string(index),
                                             getDisplayType(node->name()),
        ...
        Node *treeNode = model->AddRootEntry(
            node->value(), getDisplayType(node->next_sibling()->name()),
            node->next_sibling()->value(), node->next_sibling()->name());
        treeNode = AddNodes(treeNode, node->next_sibling(),
                            std::string(node->next_sibling()->name()));
      }
      index += 1;
    }
...
void Frame::OnFileNew(wxCommandEvent &event) {
  wxFileDialog createFileDialog(this, _("Create Property-List file"), wxEmptyString,
                              wxEmptyString, _("Property-List file|*.plist"),
                              wxFD_SAVE);
  if (createFileDialog.ShowModal() == wxID_OK) {
    std::ofstream PlistCreateFile(createFileDialog.GetPath());
    PlistCreateFile << emptyPlist;
    PlistCreateFile.close();
    std::ifstream PlistReadFile(createFileDialog.GetPath());
    std::vector<char> buffer((std::istreambuf_iterator<char>(PlistReadFile)),
                             std::istreambuf_iterator<char>());
    buffer.push_back('\0');
    doc.parse<0>(&buffer[0]);
    int index = 0;
